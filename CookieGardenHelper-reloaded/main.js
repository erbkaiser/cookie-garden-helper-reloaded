...
Game.registerMod("cookiegardenhelperreloaded", {
    ...
    //Main
    start: function () {
        this.timerId = window.setInterval(() => this.run(), this.config.timerInterval);
    },
    stop: function () {
    ...
    handleSeedClick: function (seedId) {
        if (Game.keys[17]) {
            if (!this.config.protectedSeeds.includes(seedId)) {
                ...
                this.config.protectedSeeds.splice(
                    this.config.protectedSeeds.indexOf(seedId),
                    1
                ); //deleting
            }
    ...
    handleClick: function (key) {
        if (key == "fillGardenWithSelectedSeed") {
            ...
            this.config["savedPlot"] = Game.keys[17]
                ? this.emptyPlot()
                : this.clonePlot();
    ...
    handleMouseoutPlotIsSaved: function (element) {
    ...
    handleMouseoutSeedList: function (element) {
        Game.tooltip.shouldHide = 1;
    },
    ...
    //Menu Stuff
    makeId: function (id) {
        return this.modid + this.capitalize(id);
    },
    css: function () {
        return `
    ...
    button: function (name, text, title, toggle, active) {
        if (toggle) {
            return `<a class="toggleBtn option ${
                active ? "" : "off"
            }" name="${name}" id="${this.makeId(name)}" title="${title}">
			${text}
			<span class="toggleBtnOn">ON</span>
			<span class="toggleBtnOff">OFF</span>
		  </a>`;
        }
        return `<a class="btn option" name="${name}" id="${this.makeId(
            name
        )}" title="${title}">${text}</a>`;
    },
    customButton: function (name, text, title, active) {
        return `<a class="toggleBtn option ${
            active ? "" : "off"
        }" name="${name}" id="${this.makeId(name)}" title="${title}">${text}</a>`;
    },
    toggleButton: function (name) {
        if (name == "autoPlantRotateSoilComboName") {
            this.setNextCombo();
            let btn = this.doc.qSel(
                `#cookieGardenHelperReloaded a.toggleBtn[name=${name}]`
            );
            ...
            if (name == "autoPlantRotateSoil") {
                ...
                let btn2 = this.doc.qSel(
                    `#cookieGardenHelperReloaded a.toggleBtn[name=${name2}]`
                );
    ...
    labelWithState: function (name, text, textActive, active) {
        return `<span name="${name}" id="${this.makeId(name)}" class="labelWithState ${
            active ? "active" : ""
        }"">
    ...
    labelToggleState: function (name, active) {
        let label = this.doc.qSel(
            `#cookieGardenHelperReloaded span.labelWithState[name=${name}]`
        );
        label.classList.toggle("active", active);
    },
    createWarning: function (msg) {
        this.doc.elId("row2").insertAdjacentHTML(
            "beforebegin",
            `
    ...
    readmeLink: function () {
        return "https://github.com/erbkaiser/cookie-garden-helper-reloaded/blob/master/README.md#how-it-works";
    },

    build: function () {
        if (l("cookieGardenHelperReloadedProductButton"))
            l("cookieGardenHelperReloadedProductButton").remove();
        if (l("cookieGardenHelperReloaded")) l("cookieGardenHelperReloaded").remove();

        this.doc.qSel("#row2 .productButtons").insertAdjacentHTML(
            "beforeend",
            `
			...
			<div id="cookieGardenHelperReloaded">
			  ...
			  <div id="cookieGardenHelperReloadedTools">
				  ...
				  <div class="cookieGardenHelperReloadedSubPanel">
					...
					<p>
					  ${this.button(
                          "autoHarvestMatured",
                          "Matured seeds",
                          "Harvest matured seeds as soon as they are mature",
                          true,
				  ...
				  <div class="cookieGardenHelperReloadedSubPanel">
					...
					<p>
					  ${this.button(
                          ...
                          "Clean Garden",
                          "Only allow saved and new seeds",
                          true,
				...
				<div class="cookieGardenHelperReloadedPanel" id="autoPlantPanel">
				  ...
				  <p>
					${this.button(
                        "autoPlantAvoidBuffs",
                        "Avoid Buffs",
                        "Make sure there is no buffs before planting (saving cookies)",
                        true,
				  ...
				  <p>
					${this.button(
                        "autoPlantRotateSoil",
                        "Rotate Soil",
                        "Rotate soil based on mature/young counts",
                        true,
                        this.config.autoPlantRotateSoil
                    )}
					${this.customButton(
                        "autoPlantRotateSoilComboName",
                        this.getSoilRotationCombo()[0],
                        "Cycle on possible soil rotation combos",
                        this.config.autoPlantRotateSoil
                    )}
				  </p>
				  <p>
				  ...
				  <p>
					${this.button(
                        "autoForceTicks",
                        "Force ticks",
                        "Force grow ticks to happen frequently",
                        true,
				...
				<div class="cookieGardenHelperReloadedSeedPanel" id="seedList">
				  <h2>Seed List ${this.button(
                      "ToggleSeedList",
                      "+",
                      "Display or hide seed list. In orange, what could be unlocked. In red, what cannot. Hold Ctrl and click to prevent autoHarvest."
                  )}</h2>
        ...
        if (this.isActive()) {
            for (let i = 470; i <= 476; i++) {
				...
				<div class="crate upgrade${
                    Game.UpgradesById[i].bought == 1 ? " enabled" : ""
                }" onmouseout="Game.tooltip.shouldHide=1;" onmouseover="Game.tooltip.dynamic=1;Game.tooltip.draw(this,function(){return Game.crateTooltip(Game.UpgradesById[${i}],'store');},'top');Game.tooltip.wobble();" id="upg-${i}" style="${
                    Game.UpgradesById[i].unlocked == 0 ? "opacity:.3;" : ""
                }width:48px;height:48px;background-image: url(img/icons.png);background-position: ${
                    -1 * ic[0] * 48
                }px ${-1 * ic[1] * 48}px;"></div>`;
            }
        }
    ...
    getSeedListDisplay: function () {
        ...
        if (this.isActive()) {
            str +=
                "<p>Orange - Unlockable / Red - Locked / Aura - Protection against AutoHarvest (Ctrl+Click)</p>";
            for (let i = 1; i <= this.minigame().plantsById.length; i++) {
                ...
                str += `<div class="cookieGardenHelperReloadedSmallSubPanel seedListItem" id="plant-${i}">
					<div id="gardenSeedIcon-${i}" class="cookieGardenHelperReloadedGardenSeed shadowFilter${
                    this.config.protectedSeeds.includes(i)
                        ? " cookieGardenHelperReloadedBrightenSeed"
                        : " cookieGardenHelperReloadedSeed"
                } " style="background-position:0px ${this.getSeedIconY(i)}px;"></div>
					<div style="display:inline-block;${
                        p.unlocked == 1
                            ? ""
                            : !this.parentsUnlocked(i)
                            ? "color:red;"
                            : "color:orange;"
                    }">${i} - ${p.name}</div>
				</div>`;
    ...
    getSeedIconY: function (seedId) {
        return this.getPlant(seedId).icon * -48;
    },
    ...
    buildMutationPlot: function (seedId) {
        ...
        return `<div id="cookieGardenHelperReloadedTooltip">
		  ${this.buildMutationPlotData(seedId)
              .map(
                  (row) => `<div class="gardenTileRow">
			${row
                .map(
			  ...
			  ${
                  tile[0] - 1 < 0
                      ? ""
                      : `<div class="gardenTileIcon ${
                            this.isSeedUnlocked(tile[0]) ? "on" : "off"
                        }"
				style="background-position: 0 ${this.getSeedIconY(tile[0])}px;">
			  </div>`
              }
    ...
    isSeedUnlocked: function (seedId) {
        return this.getPlant(seedId).unlocked == 1;
    },
    ...
    setCorrectSoil: function () {
        ...
        if (!M.freeze) {
            ...
            for (let x = 0; x < 6; x++) {
                for (let y = 0; y < 6; y++) {
                    if (!this.tileIsEmpty(x, y)) {
                        ...
                        if (this.getPlant(tile.seedId).immortal) continue;
            ...
            if (
                M.soil != targetSoil &&
                M.parent.amount >= M.soilsById[targetSoil].req &&
                M.nextSoil < Date.now()
            ) {
                M.nextSoil =
                    Date.now() + (Game.Has("Turbo-charged soil") ? 1 : 1000 * 60 * 10);
                M.toCompute = true;
                M.soil = targetSoil;
                M.computeStepT();
    ...
    secondsBeforeNextTick: function () {
        return (this.minigame().nextStep - Date.now()) / 1000;
    },
    selectedSeed: function () {
        return this.minigame().seedSelected;
    },
    selectSeed: function (seedId) {
    ...
    handleDying: function (plant, x, y) {
        if (!this.isExplodable(plant)) {
            if (
                this.isCpsBonus(plant) &&
                this.config.autoHarvestCheckCpSMultDying &&
                this.CpSMult() >= this.config.autoHarvestMiniCpSMultDying.value
            ) {
                this.harvest(x, y);
            } else if (
                this.config.autoHarvestDying &&
                this.secondsBeforeNextTick() <= this.config.autoHarvestDyingSeconds
            ) {
    ...
    getBuffMultCps: function () {
        ...
        for (var b in Game.buffs) {
            if (Game.buffs[b].hasOwnProperty("multCpS")) {
                mult *= Game.buffs[b].multCpS;
            }
        }
        return mult;
    },
    setPlot: function () {
        ...
        var seeds = [
            14, 13, 24, 10, 2, 9, 3, 21, 22, 23, 12, 4, 5, 7, 30, 11, 20, 18, 8, 19, 32,
            15, 17, 16, 33, 34, 29, 26, 27, 25, 28, 31, 6,
        ];
        ...
        if (this.isActive()) {
            ...
            this.forEachTile((x, y) => {
                if (this.config.autoHarvest && !this.tileIsEmpty(x, y)) {
                    ...
                    if (
                        (plant.immortal && this.config.autoHarvestAvoidImmortals) ||
                        this.config.protectedSeeds.includes(tile.seedId)
                    ) {
                        // do nothing
                    } else {
                        let stage = this.getPlantStage(tile);
